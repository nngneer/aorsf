...
double compute_mse_exported(arma::vec& y,
 ...
 List orsf_cpp(arma::mat&               x,
  ...
  // Create callbacks from R functions
  LinCombCallback lc_callback = nullptr;
  if (lincomb_type == LC_R_FUNCTION) {
    lc_callback = make_lincomb_callback(lincomb_R_function);
  } else if (lincomb_type == LC_GLMNET) {
    lc_callback = make_glmnet_callback(lincomb_R_function, lincomb_alpha, lincomb_df_target);
  }

  OobagEvalCallback oob_callback = nullptr;
  if (oobag_eval_type == EVAL_R_FUNCTION) {
    oob_callback = make_oobag_callback(oobag_R_function);
  }
   ...
   if(vi_type != VI_NONE){
    ...
    if(run_forest){
     if(vi_type == VI_ANOVA){
      ...
      uvec denom = forest->get_vi_denom();
      uvec zeros = find(denom == 0);
      if(zeros.size() > 0) denom(zeros).fill(1);

      vi_output = forest->get_vi_numer() / denom;
