...
void Forest::init(std::unique_ptr<Data> input_data,
 ...
 this->lincomb_callback = lincomb_callback;
 ...
 this->oobag_callback = oobag_callback;
 ...
 if(verbosity > 1){
  ...
  AORSF_OUT.println("------------ input data dimensions ------------");
  AORSF_OUT.println("N observations total: " + std::to_string(data->get_n_rows()));
  AORSF_OUT.println("N columns total: " + std::to_string(data->get_n_cols_x()));
  AORSF_OUT.println("-----------------------------------------------");
  AORSF_OUT.println("");
...
void Forest::grow_single_thread(vec* oobag_denom_ptr,
 ...
 for (uint i = 0; i < n_tree; ++i) {
  ...
  if(verbosity > 1){
   AORSF_OUT.println("------------ Growing tree " + std::to_string(i) + " --------------");
   AORSF_OUT.println("");
  }
  ...
  if(verbosity == 1){
   ...
   if ((progress > 0 && elapsed_time.count() > STATUS_INTERVAL) ||
    ...
    std::string msg = "Growing trees: " + std::to_string(static_cast<int>(round(100 * relative_progress))) + "%. ";
    if(progress < max_progress){
     msg += "~ time remaining: " + beautifyTime(remaining_time) + ".";
    }
    AORSF_OUT.println(msg);
 ...
 for(uint i = 0; i < n_tree; ++i){
  ...
  if(verbosity == 1){
   ...
   if ((progress > 0 && elapsed_time.count() > STATUS_INTERVAL) ||
    ...
    std::string msg = "Computing importance: " + std::to_string(static_cast<int>(round(100 * relative_progress))) + "%. ";
    if(progress < max_progress){
     msg += "~ time remaining: " + beautifyTime(remaining_time) + ".";
    }
    AORSF_OUT.println(msg);
...
void Forest::compute_dependence_single_thread(
 ...
 for (uint i = 0; i < n_tree; ++i) {
  ...
  if(verbosity > 1){
   if(oobag){
    AORSF_OUT.println("--- Computing oobag dependence: tree " + std::to_string(i) + " ---");
   } else {
    AORSF_OUT.println("------ Computing dependence: tree " + std::to_string(i) + " -----");
   }
   AORSF_OUT.println("");
  }
  ...
  if(verbosity == 1){
   ...
   if ((progress > 0 && elapsed_time.count() > STATUS_INTERVAL) ||
    ...
    std::string msg = "Computing dependence: " + std::to_string(static_cast<int>(round(100 * relative_progress))) + "%. ";
    if(progress < max_progress){
     msg += "~ time remaining: " + beautifyTime(remaining_time) + ".";
    }
    AORSF_OUT.println(msg);
...
void Forest::predict_single_thread(Data* prediction_data,
 ...
 for (uint i = 0; i < n_tree; ++i) {
  ...
  if(verbosity > 1){
   if(oobag){
    AORSF_OUT.println("--- Computing oobag predictions: tree " + std::to_string(i) + " ---");
   } else {
    AORSF_OUT.println("------ Computing predictions: tree " + std::to_string(i) + " -----");
   }
   AORSF_OUT.println("");
  }
  ...
  if(pred_type == PRED_TERMINAL_NODES){
   ...
   trees[i]->predict_value(col_i, pred_type, oobag);


  } else {

   trees[i]->predict_value(result, pred_type, oobag);
  ...
  if(verbosity == 1){
   ...
   if ((progress > 0 && elapsed_time.count() > STATUS_INTERVAL) ||
    ...
    std::string msg = "Computing predictions: " + std::to_string(static_cast<int>(round(100 * relative_progress))) + "%. ";
    if(progress < max_progress){
     msg += "~ time remaining: " + beautifyTime(remaining_time) + ".";
    }
    AORSF_OUT.println(msg);
 ...
 if (thread_ranges.size() > thread_idx + 1) {
  ...
  for (uint i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i) {
   ...
   if(pred_type == PRED_TERMINAL_NODES){
    ...
    trees[i]->predict_value(result_ptr, pred_type, oobag);
...
void Forest::show_progress(std::string operation, size_t max_progress) {
 ...
 while (progress < max_progress) {
  ...
  if ((progress > 0 && elapsed_time.count() > STATUS_INTERVAL) ||
   ...
   std::string msg = operation + ": " + std::to_string(static_cast<int>(round(100 * relative_progress))) + "%. ";
   if(progress < max_progress){
    msg += "~ time remaining: " + beautifyTime(remaining_time) + ".";
   }
   AORSF_OUT.println(msg);
