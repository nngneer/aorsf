...
#include <random>
 ...
 Tree::Tree() :
   ...
   lincomb_callback(nullptr),
   oobag_callback(nullptr),
   verbosity(0){
 ...
 void Tree::resize_oobag_data(){
  ...
  if(rows_oobag.size() == 0){
   throw computation_error("attempting to allocate oob memory with empty rows_oobag");
  }
 ...
 arma::mat Tree::glm_fit(){
  throw computation_error("default glm fit function called");
  mat out;
  return(out);
 }
 ...
 arma::mat Tree::glmnet_fit(){
  ...
  if (!lincomb_callback) {
    throw computation_error("glmnet_fit called without lincomb_callback set");
  }
 ...
 arma::mat Tree::user_fit(){
  ...
  if (!lincomb_callback) {
    throw computation_error("user_fit called without lincomb_callback set");
  }
 ...
 double Tree::evaluate_oob_with_r_function(
  ...
  if (!oobag_callback) {
    throw computation_error("evaluate_oob_with_r_function called without oobag_callback set");
  }

  return oobag_callback(y_data, w_data, pred_data);
 }
